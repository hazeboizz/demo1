...
public class TeaDao extends BaseDao {
	...
	public ArrayList<CourseBean> getCourse(String num, String year, String term) {
		String sql = "SELECT * FROM 教师任课查询  where 教师编号=? and 开课学年=? and 开课学期=?";
	...
	public ArrayList<Integer> getStuNum(String num, String year, String term) {
		ArrayList<Integer> total = new ArrayList<>();
		Connection connection;
		try {
			String sql = "exec 各教师任课的学生人数 ?,?";
			connection = dataSource.getConnection();
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, year);
			if(term.equals("上学期")) {
				boolean  i = true;
				preparedStatement.setBoolean(2, i);
			}
			else {
				boolean  i = false;
				preparedStatement.setBoolean(2, i);
			...
			String sql1 = "SELECT * FROM 教师任课的学生人数 WHERE 教师任课的学生人数.教师编号 = ?";
			preparedStatement = connection.prepareStatement(sql1);
			preparedStatement.setString(1, num);
			ResultSet resultSet = preparedStatement.executeQuery();
			while (resultSet.next()) {
				total.add(resultSet.getInt("人数"));
		   }
			connection.close();
			return total;
		} catch (SQLException e) {
